<?php
/**
 * Content analyzer for detecting schema-relevant patterns.
 *
...
/**
 * Analyzes post content to detect FAQ, HowTo, software, and video patterns.
 */
final class ContentAnalyzer {
	 ...
	 * @param string $rendered Rendered post content.
	 * @return bool
	 */
	...
	/**
	 * Check for HowTo content patterns.
	 *
	 * @param string $raw      Raw post content.
	 * @param string $rendered Rendered post content.
	 * @return bool
	...
	/**
	 * Check for software application content patterns.
	 *
	 * @param string  $raw      Raw post content.
	 * @param string  $rendered Rendered post content.
	 ...
	 * @return bool
	 */
	private function has_software_content( string $raw, string $rendered, WP_Post $post ): bool { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		$indicators = 0;
		$text       = mb_strtolower( wp_strip_all_tags( $rendered ) );

		$keywords = array( 'download', 'version', 'install', 'plugin', 'app', 'software', 'application', 'changelog', 'release notes', 'system requirements' );
	...
	/**
	 * Check for video content patterns.
	 *
	 * @param string $raw      Raw post content.
	 * @param string $rendered Rendered post content.
	 * @return bool
