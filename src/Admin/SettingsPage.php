<?php
/**
 * Per-site settings page for JSON-LD plugin.
 *
...
/**
 * Renders and handles the per-site JSON-LD settings page.
 */
final class SettingsPage {
	...
	public function register(): void {
		...
		if ( $hook ) {
			add_action( "load-{$hook}", array( $this, 'enqueue_media' ) );
		}
	}
	...
	public function render(): void {
		...
		<div class="wrap">
			...
			<?php if ( isset( $_GET['cache_flushed'] ) && '1' === $_GET['cache_flushed'] ) :  // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'JSON-LD cache flushed.', 'soderlind-json-ld' ); ?></p>
				</div>
			<?php endif; ?>
		</div>
		<?php
	...
	/**
	 * Render the Organization settings section description.
	 *
	 * @return void
	 */
	...
	public function render_org_name(): void {
		...
		$placeholder = $network['org_name'] ? $network['org_name'] : get_bloginfo( 'name' );
		printf(
	...
	public function render_org_logo(): void {
		$settings    = Settings::get_site();
		$network     = Settings::get_network();
		$placeholder = $network['org_logo'] ? $network['org_logo'] : '';
	...
	public function render_org_founding_date(): void {
		$settings    = Settings::get_site();
		$network     = Settings::get_network();
		$placeholder = $network['org_founding_date'] ? $network['org_founding_date'] : 'YYYY';
		printf(
	...
	/**
	 * Render the Social Profile URLs field.
	 *
	 * @return void
	 */
	public function render_org_social_urls(): void {
		$settings = Settings::get_site();
		$urls     = ! empty( $settings['org_social_urls'] ) ? $settings['org_social_urls'] : array( '' );
	...
	/**
	 * Add a settings field to the Organization section.
	 *
